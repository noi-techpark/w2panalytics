       ...
       function reset_single_sidebar(el){
               if(delete_sidebar_active==false){
                   ...
                   if($(el).children().length==1){
                       ...
                       var link=$(well_panel).find( "a" );
                       var length=link.length;
                       var length_link_active=0;
                       for(var i=0;i<link.length;i++){
                           if($(link[i]).children('i').hasClass('fa-check-square-o')){
                               length_link_active++;
                           }
                       }
                       setCookie("length_link_active",length_link_active,1);
                       setCookie("spinner",0,1);
                       if(length_link_active>0){
                       for(var i=0;i<link.length;i++){
                           if($(link[i]).children('i').hasClass('fa-check-square-o')){
